...
**Onde vivem:** no **projeto-alvo** (são entregáveis dele), em `docs/<raiz-configurável>/<feature-slug>/`. Raiz default proposta: `docs/sismais-dev/` (evita colisão com `docs/superpowers/specs/`); definida ao plugar no gms-mobile e configurável por projeto.
...
- **Colisão de raiz de artefatos** com `docs/superpowers/specs/` ou outros. Mitigação: raiz configurável; default `docs/sismais-dev/`.
